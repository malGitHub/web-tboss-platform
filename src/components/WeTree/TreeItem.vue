<template>
	<li v-show="model.visible" :class="['tree-parent', 'tree-parent-'+index]" :style="hasChildNodes ? '' : {'marginLeft':'20px'}">
		...
		<ul v-show="isUnfold" v-if="hasChildNodes" :class="['tree-parent-children', 'tree-parent-'+index+'-childen']">
			<TreeItem v-for='(item, index) in model[defaultOptions.children]' :layout='layout' :onlyKey='onlyKey' :index='index' :key='index'
				:model='item' :options='defaultOptions' :renderContent="renderContent" @dispathParent="changeState"></TreeItem>
		</ul>
	</li>
</template>
...
export default {
		...
		'node-content': {
			...
			render: function(h) {
				...
				return (parent.renderContent ? parent.renderContent.call(parent._renderProxy, h, {
						_self: parent.$vnode.context, node, model }) : <span class={'tree-label'}> {this.model[this.options.label]} </span>
				)
			}
		}
	},
	...
	methods: {
		...
		setCheckedClass(key, value) {
			let isWith = false
			let i = 0
			const _len = this.children.length
			if (value) {
		...
		handleSetChecked(treeModel, value) {
	...
	computed: {
	    ...
	    itemChecked: {
	    	get() {
	    		if (this.model.checkState === 'unchecked' || this.model.checkState === 'half-checked') {
	    			return false
	    		} else {
	    			return true
	    		}
